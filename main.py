...
from database import (
    init_database, get_bancos, get_categorias, get_transacoes,
...
with st.sidebar:
    ...
    st.caption("Planejador Financeiro")
    
    criar_espacamento()
    
    # Navegação
    st.subheader("🗂️ Navegação")
    páginas = ["Dashboard", "Lançamentos", "Categorias", "Bancos", "Relatórios", "Editar/Deletar"]
    
    # Obter índice da página atual
    página_atual = st.session_state.get('page', 'Dashboard')
    índice_página = páginas.index(página_atual) if página_atual in páginas else 0
    ...
    # Seleção de mês e ano (quando aplicável)
...
if page == "Dashboard":
    ...
    todas_transacoes = get_transacoes()
    saldo_acumulado = 0
    ...
    dados_ano = []
    for m in range(1, 13):
        resumo_mes = get_resumo_mes(m, ano)
        dados_ano.append({
            'Mês': meses_dict[m],
    ...
    with col1:
        ...
        gastos = get_gastos_por_categoria(mes, ano)
        ...
        if gastos:
            ...
            fig_gastos = go.Figure(data=[
                go.Bar(
                    ...
                    orientation='h',
                    marker_color=cores,
                    text=df_gastos['total'].apply(lambda x: f"R$ {x:.2f}"),
                    textposition='outside',
    ...
    with col2:
        ...
        receitas = get_receitas_por_categoria(mes, ano)
        ...
        if receitas:
            ...
            fig_receitas = go.Figure(data=[
                go.Bar(
                    ...
                    orientation='h',
                    marker_color=cores,
                    text=df_receitas['total'].apply(lambda x: f"R$ {x:.2f}"),
                    textposition='outside',
                    textfont=dict(size=11)
                )
            ])
    ...
    with col3:
        ...
        transacoes_investimento = get_transacoes(tipo='Investimento', mes=mes, ano=ano)
        ...
        if transacoes_investimento:
            ...
            fig_invest = go.Figure(data=[
                go.Bar(
                    y=df_invest_grupo['nome'],
                    x=df_invest_grupo['total'],
                    orientation='h',
                    marker_color=cores,
                    text=df_invest_grupo['total'].apply(lambda x: f"R$ {x:.2f}"),
                    textposition='outside',
                    textfont=dict(size=11)
                )
            ])
...
# PÁGINA: LANÇAMENTOS
            ...
            if valor <= 0:
                ...
                if tipo == "Despesa" and parcelas > 1:
                    ...
                    for i in range(int(parcelas)):
                        ...
                        add_transacao(tipo, categoria_id, banco_id, valor_parcela, descricao_parcela, data_parcela)
                    st.success(f"✅ {tipo} de {formatar_moeda(valor)} registrada em {int(parcelas)} parcelas!")
                else:
                    add_transacao(tipo, categoria_id, banco_id, valor, descricao, data)
    ...
    transacoes = get_transacoes(mes=mes_filtro, ano=ano_filtro)
    ...
    with col2:
        ...
        if categorias:
            ...
            if st.button("🗑️ Deletar Categoria"):
                delete_categoria(categoria_selecionada)
    ...
    with col2:
        ...
        if bancos:
            ...
            if st.button("🗑️ Deletar Banco"):
                delete_banco(banco_selecionado)
...
# PÁGINA: RELATÓRIOS
    ...
    transacoes = get_transacoes(mes=mes, ano=ano)
    if transacoes:
    ...
    if not transacoes:
        ...
        with col2:
            ...
            if st.button("🔍 Buscar", use_container_width=True):
                if transacao_id > 0:
                    transacao_temp = get_transacao_by_id(transacao_id)
                    if transacao_temp:
